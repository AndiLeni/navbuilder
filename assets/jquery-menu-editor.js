        ...
        function elFromPoint(x, y) {
            ...
            if (isRelEFP === null) {
                ...
                if ((s = state.doc.scrollTop()) > 0) {
                    isRelEFP =
                        (res = document.elementFromPoint(0, s + $(window).height() - 1)) == null ||
                        res.tagName.toUpperCase() === "HTML"; // IE8 returns html
                }
                if ((s = state.doc.scrollLeft()) > 0) {
                    isRelEFP =
                        (res = document.elementFromPoint(s + $(window).width() - 1, 0)) == null ||
                        res.tagName.toUpperCase() === "HTML"; // IE8 returns html
                }
            }
...
function MenuEditor(idSelector, options) {
    ...
    this.updateIntern = function () {
        ...
        $cEl.data(
            "text",
            $form.find("input#internLabel").val()
                ? $form.find("input#internLabel").val()
                : $form.find("input#REX_LINK_href_NAME").val()
        );
    ...
    this.addIntern = function () {
        ...
        data["text"] = $form.find("input#internLabel").val()
            ? $form.find("input#internLabel").val()
            : $form.find("input#REX_LINK_href_NAME").val();
